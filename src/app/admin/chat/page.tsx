/**
 * Chat Sessions Admin Page
 * View and manage chat conversations
 */

import Link from 'next/link';

import { createServerSupabaseClient } from '@/lib/supabase/server';
import { cn } from '@/lib/utils';
import type { ChatSession } from '@/types/database';

export const metadata = {
  title: 'Chat Messages | Admin',
};

interface PageProps {
  searchParams: Promise<{
    status?: string;
    page?: string;
  }>;
}

export default async function ChatPage({ searchParams }: PageProps) {
  const params = await searchParams;
  const status = params.status ?? 'all';
  const page = parseInt(params.page ?? '1', 10);
  const perPage = 20;

  const supabase = await createServerSupabaseClient();

  // Get sessions with message count
  let query = supabase
    .from('chat_sessions')
    .select('*', { count: 'exact' })
    .order('last_message_at', { ascending: false })
    .range((page - 1) * perPage, page * perPage - 1);

  if (status !== 'all') {
    query = query.eq('status', status as ChatSession['status']);
  }

  const { data, count } = await query;
  const sessions = data as ChatSession[] | null;
  const totalPages = Math.ceil((count ?? 0) / perPage);

  // Get unread count
  const { count: unreadCount } = await supabase
    .from('chat_messages')
    .select('*', { count: 'exact', head: true })
    .eq('is_read', false)
    .eq('sender', 'visitor');

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-neutral-900">Chat Messages</h1>
        <p className="text-neutral-600">
          {unreadCount ?? 0} unread messages
        </p>
      </div>

      {/* Filters */}
      <div className="flex flex-wrap gap-2">
        {[
          { value: 'all', label: 'All' },
          { value: 'active', label: 'Active' },
          { value: 'closed', label: 'Closed' },
          { value: 'converted', label: 'Converted' },
        ].map((s) => (
          <Link
            key={s.value}
            href={`/admin/chat?status=${s.value}`}
            className={cn(
              'rounded-full px-4 py-1.5 text-sm font-medium transition-colors',
              status === s.value
                ? 'bg-primary-100 text-primary-700'
                : 'bg-neutral-100 text-neutral-600 hover:bg-neutral-200'
            )}
          >
            {s.label}
          </Link>
        ))}
      </div>

      {/* Sessions List */}
      <div className="space-y-3">
        {sessions?.length === 0 ? (
          <p className="py-12 text-center text-neutral-500">
            No chat sessions found
          </p>
        ) : (
          sessions?.map((session) => (
            <Link
              key={session.id}
              href={`/admin/chat/${session.id}`}
              className="block rounded-xl border border-neutral-200 bg-white p-4 transition-shadow hover:shadow-md"
            >
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-medium text-neutral-900">
                    Session #{session.id.slice(0, 8)}
                  </p>
                  <p className="text-sm text-neutral-500">
                    Visitor: {session.visitor_id.slice(0, 12)}...
                  </p>
                </div>
                <div className="text-right">
                  <StatusBadge status={session.status} />
                  <p className="mt-1 text-xs text-neutral-400">
                    {new Date(session.last_message_at).toLocaleString()}
                  </p>
                </div>
              </div>
            </Link>
          ))
        )}
      </div>

      {/* Pagination */}
      {totalPages > 1 && (
        <div className="flex items-center justify-between">
          <p className="text-sm text-neutral-500">
            Page {page} of {totalPages}
          </p>
          <div className="flex gap-2">
            <Link
              href={`/admin/chat?status=${status}&page=${page - 1}`}
              className={cn(
                'rounded-lg px-4 py-2 text-sm font-medium',
                page <= 1
                  ? 'pointer-events-none opacity-50'
                  : 'bg-neutral-100 hover:bg-neutral-200'
              )}
            >
              Previous
            </Link>
            <Link
              href={`/admin/chat?status=${status}&page=${page + 1}`}
              className={cn(
                'rounded-lg px-4 py-2 text-sm font-medium',
                page >= totalPages
                  ? 'pointer-events-none opacity-50'
                  : 'bg-neutral-100 hover:bg-neutral-200'
              )}
            >
              Next
            </Link>
          </div>
        </div>
      )}
    </div>
  );
}

function StatusBadge({ status }: { status: string }) {
  const colors: Record<string, string> = {
    active: 'bg-green-100 text-green-700',
    closed: 'bg-neutral-100 text-neutral-600',
    converted: 'bg-blue-100 text-blue-700',
  };

  return (
    <span
      className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${colors[status] ?? colors.active}`}
    >
      {status}
    </span>
  );
}
